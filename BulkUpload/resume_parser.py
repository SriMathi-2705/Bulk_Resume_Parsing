...
class ResumeParser:
    ...
    @staticmethod
    def extract_resume_data(content, filename):
        """Extracts all necessary resume data."""
        resume_text = ResumeParser.extract_text_from_pdf(content)  # Directly process the file path

        # Extract various resume details
        name = ResumeParser.extract_name(resume_text)
        email = ResumeParser.extract_email_from_resume(resume_text)
        phone = ResumeParser.extract_mobile_numbers(resume_text)
        education = ResumeParser.extract_education_from_resume(resume_text)
        dob, age = ResumeParser.extract_dob_age(resume_text)
        date_of_birth = ResumeParser.format_date_to_custom(dob)
        gender = ResumeParser.extract_gender_from_resume(resume_text)
        experience = ResumeParser.extract_experience_from_resume(resume_text)
        return {
            "filename": filename,
            "name": name,
            "email": email,
